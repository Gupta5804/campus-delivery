...
const Page = () => {
  ...
  const { data: shopProducts, isLoading: shopProductsLoading } = useShopProductsQuery(currentShopId || 0);
  const config = [
    {
      label: 'First Name',
      ...
      <main className='mx-auto max-w-7xl py-6 my-8 sm:px-6 lg:px-8'>
        {user?.user_type === 'customer' ? (
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4'>
                {shopProfiles?.map((profile) => (
                  <div key={profile.id} className='bg-white p-6 rounded-lg shadow-md'>
                    ...
                    <button onClick={() => handleOpenMenu(profile.id)} className='mt-4 bg-blue-500 text-white px-4 py-2 rounded-md'>Open Menu</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
              ...
              {shopProfiles?.map((profile) => (
                <div key={profile.id} className='bg-white p-6 rounded-lg shadow-md'>
                  ...
                  <div className='mt-4 flex space-x-4'>
                    <button onClick={() => handleOpenMenu(profile.id)} className='bg-blue-500 text-white px-4 py-2 rounded-md'>Menu</button>
            ...
            <Dialog onClose={() => setIsDialogOpen(false)} className='fixed inset-0 overflow-y-auto'>
